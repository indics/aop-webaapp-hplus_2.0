Namespace.register("com.cosim.platform.desktop");
/**
 * 获取栏目模板
 */
com.cosim.platform.desktop.getColumnTemp= function(cols,width,showTable){
...
com.cosim.platform.desktop.homeTemp= function(cols,width){	
  $.post(
	__ctx+"/platform/system/desktopMycolumn/getMycolumnData.ht",
	function(jsonData){
		var colshtml=com.cosim.platform.desktop.getColumnTemp(cols,width,0);		
 ...
 * 获取栏目数据的模板
 */
com.cosim.platform.desktop.myTemp= function(cols,columnId,width,showTable,desktop){
		    var columnUrl;
	        ...
	        $.ajax({  
	            ...
	            success: function (jsonData) {  
	            	//加载布局模板
	                colshtml=com.cosim.platform.desktop.getColumnTemp(cols,width,showTable,desktop);
	                $("#colshtml").append(colshtml);
 ...
 * 保存布局
 */
com.cosim.platform.desktop.saveCol= function(layoutID,filename,layoutCols){
		var jsonSaveStr = "[";
		// 获取布局数据
		...
		$.ajax({
			...
			success : function(result) {
				var msg = new com.cosim.form.ResultMessage(result);
				if (msg.isSuccess()) {
					ref = true;
