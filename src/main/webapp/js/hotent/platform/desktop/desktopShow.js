...
function addThisColumn(desktop) {	
	if ($(".active", $("#colshtml")).length == 0) {
		$.ligerMessageBox.warn("提示信息", "请选中一列以添加栏目!");
		return;
	}
...
function saveMycolumn(u){
	...
	$.post(__ctx + u,
			...
			function(reData){				
				...
				if(parStr.result==1)
					$.ligerMessageBox.success("提示信息", parStr.message,function(){location.reload();});
				else
					$.ligerMessageBox.warn("提示信息", parStr.message,function(){location.reload();});
			});
};
