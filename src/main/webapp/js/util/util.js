...
jQuery.extend({
			 ...
			 * 
			 * @param name   字段名称
			 * @param filter 过滤条件,只允许一个条件{seq,value} AddedBy zouping
			 * @returns {String}
			 */
			getChkValueWithFilter : function(name,filter,error) {
				var str = "";
				var arr = [];
				var i = 1;
				$('input[type="checkbox"][name=' + name + ']').each(function() {
					if ($(this).attr('checked')) {
						if(filter!=undefined && filter!=null && filter!=''){
							var td = $(this).parent().parent().find('td')[filter.seq];
							if($(td).val()!=filter.value){
								arr.push(i);
							}else{
								str += $(this).val() + ",";
							}
		 ...
		 * DisplayTag工具，获取所选择行
		 */
		getSelectedRows : function(obj, selectedRows){
			//获取display表格
			var $table = obj;
			//获取表头
			var $head = $table.find('thead>tr');
			var $head_tds = $head.find('th');
			
			//获取所选择行
			var $selectedRows = selectedRows || $table.find('tbody input:checked');		
			var rows = [];
			$selectedRows.each(function(){
				...
				$tds.each(function(){	
					...
					//给<td>赋值
					var text = $($head_tds[i]).text().trim();
					if($($head_tds[i]).find('a')[0]!=undefined){
						text = $($head_tds[i]).find('a').text().trim();	
					}
					if(text!=null && text!=''){
						text = text.trim();
						row[text] = $(this).text().trim();	
					}
					i++;
				})
				rows.push(row);
...
/**
 * 对数字类型的变量handle除法操作，可以避免相除出现一长串小数位情况，调用方法，例如要计算 price*num,就可以写为price.mul(num)
 * @param arg
 * @returns
 */
