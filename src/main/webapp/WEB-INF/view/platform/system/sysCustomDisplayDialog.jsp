...
<script type="text/javascript">
	$(function(){
		$('#btnSelect').click(function(){
			...
			if($("#customDisplayListFrame").contents().find("input:[name=id]:radio:checked").length == 0){
				$.ligerMessageBox.warn('提示信息','请选择一个显示组件！');
				return false;
			}else{
				var selectEl= $("#customDisplayListFrame").contents().find("input:[name=id]:radio:checked");
				var id=selectEl.val();
				var name=selectEl.attr('dsplName');
				window.returnValue={
					status:true,
					id:id,
					name:name
...
<body>
	<div class="panel">
		...
		<div style="height:500px;">
			<iframe id="customDisplayListFrame" name="customDisplayListFrame" height="90%"
				width="100%" frameborder="0"
				src="${ctx}/platform/system/sysCustomDisplay/selector.ht"></iframe>
		</div>
