...
<%@page language="java" pageEncoding="UTF-8"
	import="com.cosim.platform.model.system.SysUser"%>
		...
		if (obj.isSuccess()) {
			$.ligerMessageBox.confirm("提示信息", obj.getMessage()+",是否继续操作", function(rtn) {
				if(rtn){
					...
					window.location.href = "${ctx}/cloud/operation/auth/user/list.ht?sysOrgInfoId=${sysOrgInfoId}&returnUserListUrl=${returnUserListUrl}";
				}
			});
		} else {
...
// 					rootPId: -1
			...
			async: {
				enable: true,
				url:__ctx + '/platform/system/grade/getOrgJsonByTenantId.ht?tenantId=${param.sysOrgInfoId}',
	...
	function loadorgGradeTree(){
		var setting = {
		   ...
		   var orgUrl=__ctx + "/platform/system/grade/getOrgJsonByTenantId.ht?tenantId=${sysOrgInfoId}";
		   //一次性加载
	...
	//生成岗位树      		
	  function loadposTree() {
		  var setting = {
					...
					async: {
						enable: true,
						url:__ctx+"/cloud/saas/position/getChildTreeDataByTenantId.ht?tenantId=${sysOrgInfoId}" ,
	 ...
	 function loadrolTree() {
		var setting = {       				    					
			...
			check: {
				...
				enable: true,
				url:'${ctx}/platform/system/sysRole/getTreeDataByTenant.ht?tenantId=${sysOrgInfoId}&systemId=${f:getCurrentSystemId()}',
							...
							<table align="center" cellpadding="0" cellspacing="0" class="table-grid">
								<tr>
									<td width="28%" valign="top"
										...
										<div class="panel-tab-div"	style="height: 300px; overflow-y: auto; border: 1px solid #6F8DC6;">
											<div id="posTree" class="ztree"	style="width: 200px; margin: -2; padding: -2;"></div>
										</div>
									</td>
											...
											<table id="posItem" class="table-grid" cellpadding="1"	cellspacing="1">
												...
												<c:forEach items="${posList}" var="posItem">
													<tr id="${posItem.posId}" style='cursor: pointer'>
														<td style="text-align: center;">${posItem.posName}<input
															type="hidden" name="posId" value="${posItem.posId}">
														</td>
														<td style="text-align: center;"><input type="radio"
															name="posIdPrimary" value="${posItem.posId}"
															<c:if test='${posItem.isPrimary==1}'>checked</c:if> /></td>
	
														<td style="text-align: center;"><a href="#"
															onclick="delrow('${posItem.posId}')" class="link del">移除</a>
														</td>
													</tr>
												</c:forEach>
											</table>
