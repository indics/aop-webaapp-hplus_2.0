    		...
    		$.ligerMessageBox.confirm('提示','确定要删除吗？此操作将删除所有子组织',function(rtn){
    			if(rtn){
    				$.ajax({ 
    				       type: "POST", 
    				       url: "orgdel.ht", 
    				       data: "orgId=${sysOrg.orgId}&sysOrgInfoId=${sysOrgInfoId}", 
    				       success: function(){ 
 ...
 <div class="panel" id="toppanel">
 		<c:choose>
		  	<c:when test="${action=='global' }">
		  		<f:tab curTab="1组织简介${sysOrg.orgId}" tabName="enterpriseOrg"/>
		  	</c:when>
		  	<c:otherwise>
		  		<f:tab curTab="2组织简介${sysOrg.orgId}" tabName="enterpriseOrgGrade"/>
		  	</c:otherwise>
		  </c:choose>
			...
			<c:choose>
				...
				<c:otherwise>
						<div class="panel-toolbar">
							<div class="toolBar">
								<div class="group"><a class="link add" href="edit.ht?orgId=${sysOrg.orgId}&demId=1&action=add&sysOrgInfoId=${sysOrgInfoId}">添加下级组织</a></div>
								<div class="l-bar-separator"></div>
								<div class="group"><a class="link save" href="edit.ht?orgId=${sysOrg.orgId}&demId=1&flag=upd&sysOrgInfoId=${sysOrgInfoId}">编辑</a></div>
								<div class="l-bar-separator"></div>
								<div class="group"><a class="link del1"  href="javascript:delOrg();">删除</a></div>
								<div class="l-bar-separator"></div>
