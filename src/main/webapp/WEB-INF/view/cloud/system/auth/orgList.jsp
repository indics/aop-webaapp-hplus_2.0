...
<script type="text/javascript" src="${ctx}/js/cosim/platform/system/SysOrgSearch.js"></script>
<script type="text/javascript">
	...
	function loadTree(selid) {
		var setting = {
				...
				async: {
					enable: true,
					url:"${ctx}/platform/system/grade/getOrgJsonByTenantId.ht?tenantId=${param.sysOrgInfoId}",
...
// 					setIcon(node);
	...
	function zTreeOnLeftClick(event, treeId, treeNode) {
		...
		$("#viewFrame").attr("src", "get.ht?orgId=" + orgId+"&sysOrgInfoId=${sysOrgInfoId}");
	};
	...
	function editNode() {
		...
		var demId = treeNode.demId;
		var url = "edit.ht?orgId=" + orgId + "&demId=" + demId + "&flag=upd&sysOrgInfoId=${sysOrgInfoId}";
		$("#viewFrame").attr("src", url);
	};
...
<!-- /////ht b --> 
			<div class="tree-toolbar" id="pToolbar">
				<div class="toolBar"
					...
					<div class="l-bar-separator"></div>
					<div class="group"><a class="link back" href="${returnUserListUrl }">返回</a></div>
				<%--
					<div class="group">
		...
		<div position="center" id="orgView" style="height: 100%;">
			...
			<iframe id="viewFrame" src="get.ht?sysOrgInfoId=${sysOrgInfoId }" frameborder="0" width="100%"
				height="100%"></iframe>
		</div>
	</div>
